...
class WeatherRepository @Inject constructor() {
    ...
    fun getCityListByName(cityName: String): Flowable<List<WeatherItem>> =
            apiService.getCityListByName(cityName, apiKey)
                    ...
                    .map { it?.list?.filter { it.sys.country != null } }
    ...
    fun getWeatherDetailsFromDb(): Single<List<ListItem>> =
            Single.just(daoSession.weatherDao)
                    ...
                    .map {
                        ...
                        items.apply {
                            add(ImageListItem(ListItemType.IMAGE_ITEM, "${it?.name}, ${it?.country}", context.getWeatherImagePath(it?.icon)))
                            add(ListItem(ListItemType.ITEM, context.getString(R.string.temperature), it?.temperature.toString()))
                            add(ListItem(ListItemType.ITEM, context.getString(R.string.pressure), it?.pressure.toString()))
                            add(ListItem(ListItemType.ITEM, context.getString(R.string.humidity), it?.humidity.toString()))
                            add(ListItem(ListItemType.ITEM, context.getString(R.string.wind_speed), it?.windSpeed.toString()))
                        }
                    }

    fun clearData(): Completable = Single.just(
            daoSession.weatherDao).map { it.deleteAll() }.toCompletable()
